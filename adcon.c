...
float iTempTot;
	R1=0;
	/*Note that if using a different Resistor then one Specified R2 value will need to be changed accordingly*/
	R2=3273;
      uint8_t id;
  	int32_t adc[8];
	int32_t volt[8];
		...
		while(1)
	{
		...
		for (i = 0; i < ch_num; i++)
		{
			...
			iTemp = volt[i];	/* uV  */
						...
						tempF = (tempK * 1.8)-459.67; //Fahrenheit
					//write data from channel 4 to file
					if(i==7 && j>5){
					/*Note I choose to write the Fahrenheit Temp to the file, other may just as easily be used*/
					/*fifo(tempC);*/
					fifo(iTempTot);
					...
					{	
						...
						printf("%f V  %f Ohms %f C %f F\r\n",iTempTot, R1, tempC, tempF);
