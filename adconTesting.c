...
//{
//	drdy_count++;
//	P2IFG &= ~0x04;						//Clear P2.2 pending interrupt
//}
...
*********************************************************************************************************
*	name: fifio
*	function:writes data to tempdata.tmp file
	Code obtained from stackoverflow.com/questions/23635576/send-data-from-c-program-to-python-program-using-pipe  
*	parameter: NULL
*	The return value:  NULL
*********************************************************************************************************
*/
int fifo7(float temp){
int s_write;
char filename[] = "ADC7.tmp";
int s_fifo = mkfifo(filename, S_IRWXU);
 	if(s_fifo !=0){
	int s_fifo = remove(filename);
	printf("mkfifo() error: %d\n" , s_fifo);
	return -1;	

	}
s_fifo = mkfifo(filename, S_IRWXU);
FILE * wfd = fopen(filename, "w");
 ...
 /* Write to FIFO */
  s_write = fprintf(wfd, "%f",temp);

	if(s_write <0){
	printf("fprintf() errpr: %d\n", s_write);
	}
	/* Close FIFO*/
fclose(wfd);
unlink(filename);
...
*********************************************************************************************************
*	name: fifoAll
*	function:writes data to plotData.csv file
	Code obtained from stackoverflow.com/questions/23635576/send-data-from-c-program-to-python-program-using-pipe  
*	parameter: NULL
*	The return value:  NULL
*********************************************************************************************************
*/
int fifoAll(float floatVoltage[8]){
int s_write;
char filename[] = "plotData.csv";
int s_fifo = mkfifo(filename, S_IRWXU);
 	if(s_fifo !=0){
	int s_fifo = remove(filename);
	printf("mkfifo() error: %d\n" , s_fifo);
...
FILE * wfd = fopen(filename, "w");
 ...
 /* Write to FIFO */
  s_write = fprintf(wfd, "%f,%f,%f,%f,%f,%f,%f,%f\n", floatVoltage[0], floatVoltage[1], floatVoltage[2], floatVoltage[3], floatVoltage[4], floatVoltage[5], floatVoltage[6], floatVoltage[7]);
...
*********************************************************************************************************
*	name: writeLine
	Writes a new line for a csv file previously opened
*	parameter: NULL
*	The return value:  NULL
*********************************************************************************************************
*//*
void writeLine(float floatVoltage[8], FILE *dataLog){
	fprintf(dataLog, "%f,%f,%f,%f,%f,%f,%f,%f\n", floatVoltage[0], floatVoltage[1], floatVoltage[2], floatVoltage[3], floatVoltage[4], floatVoltage[5], floatVoltage[6], floatVoltage[7]);
...
*/ 
	...
	//void InterruptPin1 (void) { ++globalCounter [1]; }
...
int  main()
{
	...
	bcm2835_spi_begin();
   	...
   	FILE* dataLog = fopen("Intr_dataLog1.csv", "w");
	...
	//wiringPiISR (0, INT_EDGE_FALLING, &InterruptPin0);
	//wiringPiISR (1, INT_EDGE_FALLING, &InterruptPin1);
	//wiringPiISR (2, INT_EDGE_FALLING, &InterruptPin2);
	//wiringPiISR (3, INT_EDGE_FALLING, &InterruptPin3);
	//wiringPiISR (4, INT_EDGE_FALLING, &InterruptPin4);
	//wiringPiISR (5, INT_EDGE_FALLING, &InterruptPin5);
	//wiringPiISR (6, INT_EDGE_FALLING, &InterruptPin6);
	//wiringPiISR (7, INT_EDGE_FALLING, &InterruptPin7);
	
		//while(1)		/* outter while loop */
		  //{
			  //bsp_DelayUS(1000);
				//time += 0.001;
	       		//while((ADS1256_Scan() == 0)) 	/* wait for interrupt here*/
				  //{
					//bsp_DelayUS(1000);		
					//time += 0.001;
				  //}
				...
				//for (i = 0; i < ch_num; i++)
				 //{
					...
					//floatVoltage[i]= (float)volt[i]/1000000;	
				 //}
				//printf("j=%d\n",j);
				//if ( j % 8 == 0 ){										/* funky readings--reads old values across 8 channels and only refreshes 1 per loop */
					//printf("time is %f seconds \n",time);
					////fifoAll(floatVoltage);
					//fprintf(dataLog, "%f,%f,%f,%f,%f,%f,%f,%f,%f\n", time, floatVoltage[0], floatVoltage[1], floatVoltage[2], floatVoltage[3], floatVoltage[4], floatVoltage[5], floatVoltage[6], floatVoltage[7]);
					//fflush(dataLog);
				//}
				//j++;
				////fprintf(dataLog, "%f,%f,%f,%f,%f,%f,%f,%f\n", floatVoltage[0], floatVoltage[1], floatVoltage[2], floatVoltage[3], floatVoltage[4], floatVoltage[5], floatVoltage[6], floatVoltage[7]); 	
