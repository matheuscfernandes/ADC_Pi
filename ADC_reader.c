...
*/ 
	...
	//trigger the interrupt flag, which calls the Interupt Service Routine (line 710) 
	//When triggered this will change a global variable so that the output will be written to file (in main)
...
int  main()
{
	...
	bcm2835_spi_begin();
    	...
    	bcm2835_gpio_fsel(SPICS, BCM2835_GPIO_FSEL_OUTP);				//Set SPICS as an output pin
    	bcm2835_gpio_write(SPICS, HIGH);								//Set SPICS to High
	...
	else
	{
  	...
  	ADS1256_CfgADC(ADS1256_GAIN_4, ADS1256_30000SPS);
		...
		FILE* dataLog = fopen("plotData.csv", "w");
		...
		while(1)
		{
			...
			{	
				...
				lastintrflag=1;						//here we look are passing the current interupt to lastintrflag so that we can reset intrflag after we write to the file below
				// printf("\nADC Pin: %d , \tadc val: %d \tvolt : %f \n", c, adc[c], floatVoltage[c]);
				if( c == 7 )
				{
					c = 0;
					currentTime = getCurrentTime(startTime);
					//fprintf(dataLog, "Time: %f; ch0 %f,ch1 %f,ch2 %f,ch3 %f,ch4 %f,ch5 %f,ch6 %f,ch7 %f\n", currentTime, floatVoltage[0], floatVoltage[1], floatVoltage[2], floatVoltage[3], floatVoltage[4], floatVoltage[5], floatVoltage[6], floatVoltage[7]);
					fprintf(dataLog, "%f,%f,%f,%f,%f,%f,%f,%f,%f\n", currentTime, floatVoltage[0], floatVoltage[1], floatVoltage[2], floatVoltage[3], floatVoltage[4], floatVoltage[5], floatVoltage[6], floatVoltage[7]);
					//removed labels from the line above to accomodate ease of output to csv file so data can be read from it easier (text in the fprintf statement makes the output a string instead of a float)
					fflush(dataLog);
				}
				else
				{
			...
			if (intrflag != 0 && lastintrflag != 0)		//reset intrflag to 0 if so that we can catch the next interupt while remembering we just had one (lastintrflag)
			{
				intrflag=0;
			}
							
			else if( ( intrflag == 0 && lastintrflag == 1 ) || ( intrflag == 0 && lastintrflag == 0 ) )	//if we havent had an interupt in a while ensure the lastintrflag goes back to 0
			{
